<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if($cek_user_result){
            ...
            $query_insert_user = mysqli_query($connection, "INSERT INTO user VALUES('','$full_name','$email','$phone','$address', '$password', NOW(), 1)");
            if($query_insert_user){
                $response['value'] = 1;
                $response['message'] = 'Yes, Registration is successful. Please log in with your account';
                echo json_encode($response);
            }else{
                $response['value'] = 2;
